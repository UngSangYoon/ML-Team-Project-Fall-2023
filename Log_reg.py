...
class LogisticRegression:
    ...
    def learn(self, lr, step, batch_size):
        ...
        for i in range(step):
            ...
            for j in range(jlen):
                dt = (
                    self.sigmoid(x_batch, self.weights[:, j]) - y_batch[:, j]
                ).reshape(batch_size, 1) * x_batch
                # 그래디언트 백터 계산
...
def run():
    ...
    # split datasets
    (x_train, y_train), (x_test, y_test), (x_eval, y_eval) = train_test_eval_split(
        stats, scores, test_ratio=0.1, eval_ratio=0.1
    )
    ...
    weights, costs, eval_costs = log.learn(0.00001, 5000, 200)
    pre_cnt = 0
    pre_wr = 0
